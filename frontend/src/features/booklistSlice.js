...
export const fetchBooks = createAsyncThunk("books/fetchBooks", async () => {
    const response = await axios.get("https://redux-prep-1.vercel.app/books")
    return response.data
})

export const addBook = createAsyncThunk("book/addBook", async (bookData) => {
    const response = await axios.post("https://redux-prep-1.vercel.app/books", bookData)
    return(response.data)
})

export const deleteBook = createAsyncThunk("book/deleteBook", async (bookId) => {
    const response = await axios.delete(`https://redux-prep-1.vercel.app/books/${bookId}`)
    return response.data
})

export const updateBook = createAsyncThunk("book/updateBook", async ({bookId, updatedData}) => {
    const response = await axios.patch(`https://redux-prep-1.vercel.app/book/update/${bookId}`, updatedData)
    return response.data
})
